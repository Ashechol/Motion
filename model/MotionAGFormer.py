...
from model.modules.mlp import MLP, MLPGCN
...
class GraphMLPFormerBlock(nn.Module):
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
        ...
        self.graph_mlp = MLPGCN(dim, int(dim * mlp_ratio), act_layer, n_frames,
                                drop, drop_path, use_layer_scale, layer_scale_init_value)
...
def create_layers(dim, n_layers, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop_rate=0., drop_path_rate=0.,
    ...
    for _ in range(n_layers):
        layers.append(GraphMLPFormerBlock(dim=dim,
                                          mlp_ratio=mlp_ratio,
                                          act_layer=act_layer,
                                          attn_drop=attn_drop,
                                          drop=drop_rate,
